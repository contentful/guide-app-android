...
public class InfoActivity extends Activity implements View.OnClickListener {
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    Picasso.with(this).load(place.getPictures().get(0).getUrl()).fit().centerCrop().into(ivPhoto);
  ...
  private void onEmailClicked() {
    startActivity(
        new Intent(Intent.ACTION_SENDTO, Uri.fromParts("mailto", place.getEmail(), null)));
  }
  ...
  private void onCallClicked() {
    startActivity(
        new Intent(Intent.ACTION_VIEW).setData(Uri.fromParts("tel", place.getPhoneNumber(), null)));
  }
  ...
  private void onMapClicked() {
    startActivity(new Intent(this, MapActivity.class).putExtra(InfoActivity.EXTRA_PLACE, place));
  }
  ...
  private void onWebsiteClicked() {
    startActivity(new Intent(Intent.ACTION_VIEW).setData(Uri.parse(place.getUrl())));
  }
}
